...
AUTHOR = f"{os.environ.get('SCENETEST_AUTHOR_CN', 'Presenter')}  {os.environ.get('SCENETEST_STUDENT_ID', 'Student ID')}"
FONT = "PingFang SC"
MONO_FONT = "Menlo"
...
def build():
    ...
    metric_card(slide, 5.25, 5.72, "311/311", "修复后测试全部通过", COLORS["green"])
    metric_card(slide, 8.05, 5.72, "+32.8pt", "总体通过率提升", COLORS["blue"])
    ...
    slide = prs.slides.add_slide(blank)
    base_slide(slide, "03 系统架构", "三个 Agent 加一个测试闭环", "DeepSeek 用于真实 Contract Agent；离线 parser 保证仓库可复现。")
    flow(slide, ["Contract Agent", "Code Agent", "SceneBuilder", "Test Compiler", "Test Runner", "Repair Agent"], 0.75, 2.35, 1.48)
    ...
    slide = prs.slides.add_slide(blank)
    base_slide(slide, "08 定量结果", "20 个 prompt、311 个测试，修复后全部通过", "对比 single-pass、contract-only 和完整 SceneTest。")
    ...
    metric_card(slide, 2.05, 5.55, "93", "object tests", COLORS["blue"])
    metric_card(slide, 5.48, 5.55, "73", "relation tests", COLORS["amber"])
    metric_card(slide, 8.91, 5.55, "93", "visibility tests", COLORS["teal"])
    ...
    slide = prs.slides.add_slide(blank)
    base_slide(slide, "10 可靠性与限制", "当前版本强调可复现闭环，而不是追求最高视觉真实感", "这些限制已写入报告，避免过度声称。")
    panel(slide, 0.92, 2.05, 3.35, 2.25, "为什么结果可信", "测试来自 contract，failure report 可复查；benchmark 可一键复跑。", COLORS["panel"], COLORS["blue"])
    panel(slide, 5.0, 2.05, 3.35, 2.25, "实现取舍", "主测试后端是 SceneBuilder；Blender 用于最终渲染，不在 Blender 内部跑所有测试。", COLORS["soft_teal"], COLORS["teal"])
    panel(slide, 9.08, 2.05, 3.35, 2.25, "未来增强", "加入 Blender segmentation mask、遮挡判断、更开放的物体库和更大 prompt set。", COLORS["soft_amber"], COLORS["amber"])

    slide = prs.slides.add_slide(blank)
    base_slide(slide, "11 总结", "SceneTest 让文本到 3D 场景生成变得可检查、可测试、可修复", "项目满足 Agent + 图形学主题，并提供实现、实验、公开代码和现场演示。")
    panel(slide, 1.12, 2.1, 3.25, 2.0, "贡献 1：Scene Contract", "把自然语言需求转成结构化约束。", COLORS["panel"], COLORS["blue"])
    panel(slide, 5.05, 2.1, 3.25, 2.0, "贡献 2：Graphics Unit Tests", "验证对象、关系、可见性、材质和光照。", COLORS["soft_teal"], COLORS["teal"])
    panel(slide, 8.98, 2.1, 3.25, 2.0, "贡献 3：Failure-Guided Repair", "失败测试触发局部修复并重新测试。", COLORS["soft_amber"], COLORS["amber"])
